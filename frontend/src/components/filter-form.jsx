...
const FilterForm = ({ filters, setFilters, setFilterFormOpen }) => {
  ...
  return (
    ...
    <form>
      ...
      <input type="checkbox" disabled={isLoggedIn} value="" onChange={(e) => setFilters({...filters, only_show_followed: e.target.checked })} className="sr-only peer"/>
    ...
    <input
      ...
      value = {author}
      onChange={(e) => { setAuthor(e.target.value); setFilters({ ...filters, author: e.target.value })}}
    />
    <div>
      Licence
      <ChipSet
        values={Object.keys(licenceValues)}
        selected={licence}
        setSelected={(v) => { setLicence(v); setFilters({...filters, licence: licenceValues[v]}); }}
      />
    </div>
    <div>
      Order By
      <ChipSet
        values={Object.keys(orderByValues)}
        selected={orderBy}
        setSelected={(v) => { setOrderBy(v); setFilters({...filters, order_by: orderByValues[v]}); }}
      />
    </div>
    <div>
      Order
      <ChipSet
        values={Object.keys(ascendingValues)}
        selected={ascending}
        setSelected={(v) => { setAscending(v); setFilters({...filters, ascending: ascendingValues[v]}); }}
      />
    </div>
    </div>
  );
